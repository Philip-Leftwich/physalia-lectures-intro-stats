...
[Crameri et al 2020](https://www.nature.com/articles/s41467-020-19160-7)
...
```{r, eval = TRUE, echo = F, fig.asp = 0.8, fig.width = 7}
ggplot(data, aes(x=Category, y=Value)) +
geom_col() +
theme_minimal() +
ggtitle("Better choice: bar chart")
...
```{r, echo = TRUE, eval = FALSE, fig.asp = 0.8, fig.width = 7}
ggplot(penguins, aes(bill_length_mm, bill_depth_mm,
colour=species, shape=species)) +
geom_point(size=2) +
scale_colour_discrete_qualitative()+
...
```{r, echo = FALSE, eval = TRUE, fig.asp = 0.8, fig.width = 7}
ggplot(penguins, aes(bill_length_mm, bill_depth_mm,
colour=species, shape=species)) +
geom_point(size=2) +
scale_colour_discrete_qualitative()+
...
- This may or may not be a sensible assumption so watch out! 

    Just because a Poisson distribution *usually* fits well for count data, doesn't mean that a Gaussian distribution *can't* always work.
...
The key feature of a linear mixed-effects model is the inclusion of 

- **random effects** - observations are grouped into subcategories that systematically affect our outcome.

- This accounts for **structure** in our data where some observations are more closely related than others

Mixed-effects model can be used when this structure may be important:
...
- Biological datasets are often highly structured, containing clusters of non-independent data, and LMMs allow us to explicitly model this. 
...
- Random effects represent grouping and allow the estimation of variance *within and among* these groups. 
