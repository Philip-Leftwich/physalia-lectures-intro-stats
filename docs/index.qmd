...
## Kellner et al 2025
93% of R scripts associated with species distribution and abundance papers failed to execute

Reproducibility assessment had to be abandoned due to non-functional code
...
- Comprehensive packages via CRAN, Bioconductor, and GitHub

- Explicit data manipulation pipelines (e.g. dplyr, tidyr)

- Script-based statistical modelling

- Programmatic, reproducible visualisation (e.g. ggplot2)

## Main Features of RStudio

- Four-pane interface (script, console, environment, files)

- Integrated package management

- Project-based organisation of analyses
...
Relevance: Underpins many statistical tests and probability calculations 
...
> What would have happened to the outcome if the exposure had been different, all else equal?

Linear regression can support causal estimation only when correctly specified with respect to the underlying causal structure.
...
DAGs provide a formal framework to:
...
They make assumptions explicit and open to critique.
...
- Including the confounder `z` supports unbiased estimation of the effect of `x` on `y`.

- Identifying causal structure is based on prior knowledge and expertise

- Ignoring causal structure can lead to biased research

## Antibiotic Treatment & Recovery

A hospital wants to know if a new antibiotic reduces infection recovery time. Doctors tend to prescribe this drug only to the patients with the most severe infections - when prescribed this drug they are also often recommended to be moved to intensive care.
...
We want to know if exercise reduces heart disease risk. Age affects both exercise habits (younger people exercise more) and heart disease risk. Exercise might work through weight loss, which is on the causal pathway.
...
Researchers study whether organic fertiliser increases crop yield. They measure soil quality, rainfall, and pest damage. Pest damage is influenced by both the fertiliser (which may attract pests) and rainfall (which affects pest populations).
