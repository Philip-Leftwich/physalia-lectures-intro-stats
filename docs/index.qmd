...
subtitle: " A guide for Biologists and Ecologists<br><small>24th June 2025</small>"
...
- We have some introductory R exercises:
...
- Homoscedasticity: The variance of one variable is similar across the values of the other.
...
- ${t=}{mean~difference\over~SE}{=-2.617\over1.074}{-2.437}$ 

- Degrees of freedom (df) = 28

- Critical t-value at 𝛼 = 0.05 (two-tailed) = ±2.048
...
df <- 28
t_obs <- -2.437
t_crit <- qt(0.975, df)  # Two-tailed 5% alpha level
...
# Create shading for critical regions only (tails)
t_dist$region <- ifelse( t_dist$x <= -t_crit, "Critical", "Non-critical")

# Plot
ggplot(t_dist, aes(x = x, y = y)) +
    geom_area(data = subset(t_dist, region == "Critical"), aes(fill = region), alpha = 0.4) +
    geom_line(color = "black", size = 1) +
    geom_vline(xintercept = t_obs, color = "red", linetype = "dashed", size = 1) +
    geom_vline(xintercept = c(-t_crit, t_crit), linetype = "dotted") +
    scale_fill_manual(values = c("Critical" = "cyan")) +
    labs(
        title = "t-Distribution with df = 28",
        subtitle = "Shaded: Critical Regions (|t| > 2.048); Red line: Observed t = -2.437",
        x = "t-value",
        y = "Density",
        fill = ""
    ) +
...
- Assumption check: In linear regression, one key assumption is normally distributed residuals.

- If residuals are non-normal, traditional inference methods (e.g. t-tests, CIs) may be invalid.

- Solution: Use case bootstrapping to generate robust, non-parametric estimates.
...
# Simulate data with non-normal residuals
set.seed(123)
n <- 100
x <- rnorm(n)
y <- 3 * x + rt(n, df = 2)  # Heavy-tailed residuals (non-normal)
...
boot_coefs <- replicate(n_boot, {
  ...
  boot_data <- data.frame(x = x[indices], y = y[indices])
  coef(lm(y ~ x, data = boot_data))
})
...
# Calculate 95% bootstrap CI for slope
slope_ci <- quantile(boot_df$Slope, probs = c(0.025, 0.975))
slope_ci
# Example output: [2.75, 3.35] depending on random seed

# Plot histogram of slope estimates with 95% CI
ggplot(boot_df, aes(x = Slope)) +
  geom_histogram(bins = 30, fill = "skyblue", color = "black", alpha = 0.8) +
  geom_vline(xintercept = coef(model)["x"], color = "red", linetype = "dashed", size = 1) +
  geom_vline(xintercept = slope_ci, color = "darkgreen", linetype = "dotted", size = 1) +
  labs(
    title = "Bootstrap Distribution of the Slope (Case Resampling)",
    subtitle = "Red = OLS estimate | Green = 95% Bootstrap CI",
    x = "Slope Coefficient",
    y = "Frequency"
  ) +
  theme_minimal(base_size = 14)
...
# Simulate data (non-normal, two groups)
set.seed(123)
group <- rep(c("A", "B"), each = 30)
values <- c(rnorm(30, mean = 5), rt(30, df = 2) + 6)  # Group B has non-normal data
data <- data.frame(group, values)

# Observed difference in means
obs_diff <- with(data, mean(values[group == "B"]) - mean(values[group == "A"]))

# Permutation test
n_perm <- 5000
perm_diffs <- replicate(n_perm, {
  perm_group <- sample(data$group)
  mean(values[perm_group == "B"]) - mean(values[perm_group == "A"])
})

# Create data frame for plotting
perm_df <- data.frame(perm_diffs)

# Plot null distribution and observed difference
ggplot(perm_df, aes(x = perm_diffs)) +
  ...
  labs(
    title = "Permutation Test: Null Distribution of Mean Differences",
    subtitle = paste("Red line = Observed difference =", round(obs_diff, 2)),
...
## Discussion on paired design and variable selection
...
Choose the power value (λ) that produces the best model-fit
...
- Also called heteroscedasticity-consistent (HC) standard errors

- Used when the assumption of constant variance (homoscedasticity) is violated

- Helps provide valid inference (e.g., confidence intervals, p-values) even with heteroscedasticity
...
Ω is a diagonal matrix with squared residuals or functions of them on the diagonal, allowing for heteroscedasticity.
...
| Holm's    | $p_{(i)} < \alpha/(k–i) = \alpha/k$      | we start with the smallest p-value (i = 1) and determine whether there is a significant result (i.e. p1 < α/(k–1+1) = α/k. If so we move on to the second test. We continue in this manner until we get a non-significant result
...
| Benjamin-Hochberg | $p_{(i)} \le \frac{i}{m} \cdot \alpha$ | 
...
## Formward/Backward/Stepwise-selection

- These are popular tools for simplifying models and removing non-significant predictors

- However standard statistical tests do not account for sequential testing

- models can have inflated p-values and narrow confidence intervals
...
- When we add these random variance components it adds uncertainty

- Standard t/F assumptions don't hold

- Degrees of freedom are absent or approximate

## Satterthwaite method

$$\text{df} \approx \frac{2 \cdot (\text{Var})^2}{\text{Var}(\text{Var})}$$

- Var: This is the estimated variance of a quantity (like a fixed effect estimate).

- Var of Var: This is the uncertainty in that variance estimate — how much the estimated variance might vary.

We are estimating variance of fixed effects - and that estimate has its own uncertainty - lower df = wider confidence intervals. 

**Method of calculating degrees of freedom will affect CI and inference**
